...
#	;;
	DirKernelIMG)
		$BB echo "/dev/block/platform/dw_mmc.0/by-name/BOOT";
	;;
	DirCPUGovernor)
...
#	;;
	...
	;;
	GPUFrequencyList)
		for GPUFREQ in `$BB cat /sys/devices/platform/mali.0/dvfs_table | $BB tr ' ' '\n' | $BB sort -u` ; do
		LABEL=$GPUFREQ;
...
#	;;
	...
	;;
	LiveCPUTemperature)
		CPU_R=`$BB cat /sys/class/thermal/thermal_zone0/temp`;
		CPU_C=$((CPU_R / 1000));
	...
	MinFreqIndex)
		...
		while read FREQ TIME; do
			LABEL=$((FREQ / 1000));
			if [ $FREQ -gt "100000" ] && [ $ID -le $MAXID ]; then
